...
scp a.txt ducndc@172.0.0.1:~
scp -P 2288 a.txt ducndc@172.0.0.1:~       # using a custom port
 
# copying a local file from a remote destination to the current directory
scp -P 2290 ducndc@172.0.0.1:~/a.txt .
 
# copying a local directory to a remote destination (-r)
scp -P 2290 -r projects/ ducndc@172.0.0.1:~
