...
/**
 * This function we will replace sys_openat (the function called when you call
 * the open system call) with. To find the exact prototype, with the number and type
 * of arguments, we find the original function first (it is at fs/open.c).
 *
 * In theory, this means that we are tied to the current version of the kernel. In
 * practice, the system calls almost never change (it would wreck havoc and require
 * programs to be recomplied, since the system calls are the interface between the 
 * kernel and the processes).
 */
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
